...
OUT="/storage/emulated/0/outbox/time-journal/time-journal-arm64-release.apk"
...
while (( round < MAX_ROUNDS )); do
  ...
  if ls "$ROOT/app/build/app/outputs/flutter-apk/"*.apk >/dev/null 2>&1; then
    ...
    [[ -f "$OUT" ]] && { log "DONE after copy"; exit 0; }
  fi
